...
/**
 * This is a reflective debugging helper for the {@link Config} class that may be of assistance in
 * debugging issues loading a configuration for the plugin.
 */
public final class ConfigDebugHelper {
  ...
  public static String dumpConfig(Config config)
    ...
    final StringBuilder sb =
        new StringBuilder("Dumping Config object with identity hash code ")
    ...
    for (Field field : configFields) {
      ...
      String isAssignableToIntPrimitive =
          int.class.isAssignableFrom(field.getType()) ? "yes" : "no";
      String isAssignableToBooleanPrimitive =
          boolean.class.isAssignableFrom(field.getType()) ? "yes" : "no";
    ...
    if (fieldCount != getterCount) {
      ...
      List<String> getterNames =
          configGetters.stream().map(Method::getName).collect(Collectors.toList());

      List<String> fieldNames =
          Arrays.stream(configFields).map(Field::getName).collect(Collectors.toList());
